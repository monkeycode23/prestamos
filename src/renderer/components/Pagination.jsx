...
const Page = ({ number }) => {
  ...
  return (<li key={number} className={`flex h-10 w-10 items-center justify-center rounded-lg text-sm font-medium text-gray-700 hover:bg-brand-500 hover:text-white dark:text-gray-400 dark:hover:text-white`}>{number}</li>)
}
...
function Pagination() {
  ...
  const pagination = useSelector(state => state.pagination)
  const pages = Array.from({ length: pagination.totalPages }, (_, i) => i); // Ejemplo: números 0 al 4
  ...
  return (
    <div className="flex items-center justify-between gap-4 px-6 py-4 sm:justify-normal">
      ...
      <ul className="hidden items-center gap-0.5 sm:flex">
        {
          pages.map(pageNumber => (<Page number={pageNumber + 1}></Page>))
        }
      </ul>
      <button class="flex items-center gap-2 rounded-lg border border-gray-300 bg-white px-2 py-2 text-sm font-medium text-gray-700 shadow-theme-xs hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-gray-200 sm:px-3.5 sm:py-2.5">
